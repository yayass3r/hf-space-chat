...
import { DEFAULT_SETTINGS, type SiteSettings, type SiteSettingKey } from "@/lib/types";
...
export default function AdminDashboard({ onClose }: { onClose: () => void }) {
  ...
  const saveSetting = async (key: SiteSettingKey, value: string) => {
    if (!supabase) return;
    await supabase
      .from("site_settings")
  ...
  const handleSaveSettings = async () => {
    ...
    setMessage(null);
    try {
      const entries = Object.entries(settings) as [SiteSettingKey, string][];
          ...
          {activeTab === "settings" && (
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-5 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-700 bg-slate-800 text-white focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm"
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1.5">
                      بريد المسؤولين (مفصول بفواصل)
                    </label>
                    <input
                      type="text"
                      value={settings.admin_emails}
                      onChange={(e) => updateSetting("admin_emails", e.target.value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      type="url"
                      value={settings.hf_space_url}
                      onChange={(e) => updateSetting("hf_space_url", e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.hf_api_path}
                      onChange={(e) => updateSetting("hf_api_path", e.target.value)}
              ...
              <button
                ...
                disabled={saving}
                className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-medium shadow-lg shadow-orange-500/20 hover:shadow-orange-500/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-5 space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => updateSetting("adsense_enabled", settings.adsense_enabled === "true" ? "false" : "true")}
                    className={`relative w-14 h-7 rounded-full transition-colors ${
                      settings.adsense_enabled === "true" ? "bg-orange-500" : "bg-slate-700"
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${
                        settings.adsense_enabled === "true" ? "right-0.5" : "right-7"
                      }`}
                    />
                  </button>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateSetting("adsense_ad_slot", e.target.value)}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">إعلانات التطبيقات والموبايل</p>
                    </div>
                  </div>
                  <button
                    onClick={() => updateSetting("admob_enabled", settings.admob_enabled === "true" ? "false" : "true")}
                    className={`relative w-14 h-7 rounded-full transition-colors ${
                      settings.admob_enabled === "true" ? "bg-orange-500" : "bg-slate-700"
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${
                        settings.admob_enabled === "true" ? "right-0.5" : "right-7"
                      }`}
                    />
                  </button>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateSetting("admob_app_id", e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={settings.admob_ad_unit_id}
                        onChange={(e) => updateSetting("admob_ad_unit_id", e.target.value)}
          ...
          {activeTab === "users" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-500">عرض المستخدمين وإدارة صلاحياتهم</p>
              </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-800">
                        <th className="text-right px-5 py-3 text-xs font-medium text-slate-500 uppercase">البريد الإلكتروني</th>
                        <th className="text-right px-5 py-3 text-xs font-medium text-slate-500 uppercase">الدور</th>
                        <th className="text-right px-5 py-3 text-xs font-medium text-slate-500 uppercase">تاريخ التسجيل</th>
                        <th className="text-right px-5 py-3 text-xs font-medium text-slate-500 uppercase">إجراء</th>
                      </tr>
                    </thead>
                    <tbody>
                      {users.length === 0 ? (
                        ...
                        users.map((u) => (
                          <tr key={u.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                            <td className="px-5 py-3 text-sm text-white" dir="ltr">{u.email}</td>
                            ...
                            <td className="px-5 py-3">
                              ...
                              >
                                {u.role === "admin" ? "إلغاء الإدارة" : "ترقية لمسؤول"}
                              </button>
                            </td>
                          </tr>
                        ))
