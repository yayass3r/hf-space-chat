...
export default function ChatApp({ onAdminClick }: { onAdminClick: () => void }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (contentType.includes("text/event-stream") || contentType.includes("text/plain")) {
        ...
        // JSON response (original behavior)
  ...
  const retryLastMessage = async () => {
    ...
    // Send again after a small delay
    setTimeout(async () => {
      ...
      try {
        ...
        if (currentSessionId) {
          await supabase?.from("ai_chat_messages").insert([
            { project_id: currentSessionId, role: "user", content: trimmed },
            { project_id: currentSessionId, role: "assistant", content: assistantContent },
          ]);
        }
      } catch (err) {
  ...
  const exportChat = () => {
    ...
    const content = messages
      .map((m) => `**${m.role === "user" ? "أنت" : "HF Space"}:**\n${m.content}`)
      .join("\n\n---\n\n");
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3">
            ...
            {sessions.length > 3 && (
              <div className="relative">
                <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                ...
                <input
                  ...
                  className="w-full pr-9 pl-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {filteredSessions.length === 0 && (
              <p className="text-xs text-slate-400 dark:text-slate-500 text-center mt-8 px-4">
                {searchQuery ? "لا توجد نتائج" : "لا توجد محادثات محفوظة بعد"}
              </p>
            )}
            {filteredSessions.map((session) => (
              ...
              >
                <button
                  ...
                  className="flex-1 text-right px-3 py-2.5 text-sm truncate"
                >
                ...
                {deleteConfirm === session.id ? (
                  ...
                  <button
                    ...
                    className="p-1.5 rounded opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all"
                    title="حذف المحادثة"
                  >
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-2 rounded-lg text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-30"
                  ...
                  {message.role === "assistant" && message.content && !message.content.startsWith("❌") && (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1 rounded text-slate-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors"
                        title="نسخ"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                      ...
                      {message.content.startsWith("❌") && index === messages.length - 1 && (
                        <button
                          ...
                          className="p-1 rounded text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors"
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl px-4 py-3 border border-slate-200 dark:border-slate-700 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-2 py-1 rounded-lg text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 border border-red-200 dark:border-red-800 transition-colors"
                  >
                    إيقاف
                  </button>
                </div>
        ...
        {error && (
          <div className="mx-4 sm:mx-6 mb-2 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm flex items-center justify-between">
            ...
            <button
              onClick={retryLastMessage}
              className="px-3 py-1 rounded-lg text-xs font-medium bg-red-100 dark:bg-red-800 hover:bg-red-200 dark:hover:bg-red-700 transition-colors"
            >
              إعادة المحاولة
            </button>
          </div>
        ...
        <div className="px-4 sm:px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
          <div className="flex items-end gap-3 max-w-4xl mx-auto">
            ...
            {isLoading ? (
              <button
                onClick={stopGeneration}
                className="flex items-center justify-center w-12 h-12 rounded-xl bg-red-500 text-white shadow-lg shadow-red-500/20 hover:shadow-red-500/40 transition-all"
                title="إيقاف"
              >
